...
export default function Index({ outlets, unpaidAssets, cashAccounts }) {
    const [payingAsset, setPayingAsset] = useState(null);
    ...
    const openPayModal = (asset) => {
        setPayingAsset(asset);
        setData({
            outlet_id: outlets[0]?.id ?? '',
            fixed_asset_id: asset.fixed_asset_id,
            date: new Date().toISOString().slice(0, 10),
            amount: asset.remaining,
            cash_account_code: cashAccounts[0]?.code ?? '',
            memo: '',
        });
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('aset.payments.store'), {
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800">
                    Pelunasan Hutang Aset
                </h2>
            }
        >
            <Head title="Pelunasan Hutang Aset" />
                                ...
                                {unpaidAssets.map((asset) => (
                                    <tr key={asset.fixed_asset_id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-gray-600">
                                            {formatRupiah(asset.asset_total)}
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-gray-600">
                                            {formatRupiah(asset.paid)}
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium text-gray-900">
                                            {formatRupiah(asset.remaining)}
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4 text-sm">
                                            <span
                                                className={`rounded-full px-2 py-1 text-xs font-medium ${statusLabel[asset.status]?.className}`}
                                            >
                                                {statusLabel[asset.status]?.text}
                                            </span>
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                                            <SecondaryButton onClick={() => openPayModal(asset)}>
                                                Bayar
                                            </SecondaryButton>
                                        </td>
                                    </tr>
                                ...
                                {unpaidAssets.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="px-6 py-6 text-center text-sm text-gray-500">
                                            Tidak ada hutang aset yang belum lunas.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <Modal show={payingAsset !== null} onClose={closeModal} maxWidth="md">
                {payingAsset && (
                    <form onSubmit={submit} className="p-6">
                        <h3 className="text-lg font-medium text-gray-900">
                            Bayar: {payingAsset.name}
                        </h3>
                        <p className="mt-1 text-sm text-gray-500">
                            Sisa hutang: {formatRupiah(payingAsset.remaining)}
                        </p>
